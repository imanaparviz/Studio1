...
import type { ProjectData, PflichtenheftOutput as PflichtenheftOutputType } from '@/types';
...
const ProjectDataSchema = z.object({
  ...
  mainComponents: z.array(z.string()).describe("List of main software components or modules."),
  stakeholders: z.array(z.string()).describe("List of key stakeholders involved in the project."),
  phases: z.array(ProjectPhaseSchema).describe("List of project phases with their start and end dates."),
});
...
const PflichtenheftSectionSchema = z.object({
  ...
  content: z.string().describe("The detailed content for this section, formatted as Markdown. Be comprehensive and professional."),
});

const PflichtenheftOutputSchema = z.object({
  einleitung: PflichtenheftSectionSchema.describe("Einleitung (Introduction): Project title, overall project goal, client/contractor if inferable."),
  ziele: PflichtenheftSectionSchema.describe("Ziele (Goals): Specific, measurable, achievable, relevant, and time-bound (SMART) goals, and explicit non-goals."),
  zielgruppen: PflichtenheftSectionSchema.describe("Zielgruppen/User (Target Groups/Users): Detailed description of target users, their needs, and the context/environment where the product will be used."),
  funktionalitaet: PflichtenheftSectionSchema.describe("Funktionalität (Functionality - Was): Key functional requirements, detailing what the system will do. Expand on provided main components with specific functionalities. Example modules: Login, Payment System, User Management, Database interactions, etc."),
  nichtFunktionaleAnforderungen: PflichtenheftSectionSchema.describe("Nicht funktionale Anforderungen (Non-functional Requirements): Aspects like performance (response times, scalability), security (data protection, access control), usability, reliability, maintainability, SEO, interfaces to other systems, etc."),
  architektur: PflichtenheftSectionSchema.describe("Architektur (Architecture): High-level overview of the system architecture, key modules and their interactions, technologies to be used (programming languages, frameworks, databases)."),
  projektabschluss: PflichtenheftSectionSchema.describe("Projektabschluss (Project Completion): Criteria for project completion, deliverables, acceptance criteria by the client, and a basic testing strategy (unit tests, integration tests, user acceptance tests)."),
  zeitplanung: PflichtenheftSectionSchema.describe("Zeitplanung (Timeline): Summary of the project timeline based on the provided phases, identification of key milestones, and dependencies between tasks/phases if inferable."),
});
export type PflichtenheftOutput = PflichtenheftOutputType; // Use the existing type from @/types

export async function generatePflichtenheft(input: ProjectData): Promise<PflichtenheftOutput> {
  // Map ProjectData to ProjectDataForAI if necessary, here they are compatible
  return generatePflichtenheftFlow(input as ProjectDataForAI);
}
...
const generatePflichtenheftFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (!output) {
      throw new Error("AI did not return an output for Pflichtenheft generation.");
    }
    return output;
  }
);
