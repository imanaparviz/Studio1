...
import { Download, Image as ImageIcon, FileText, Loader2, Info } from "lucide-react";
...
export function DiagramDisplay({ title, type, content, filenamePrefix = "diagram" }: DiagramDisplayProps) {
    ...
    const renderDiagram = async () => {
      if (type === "mindmap" && svgRef.current && content) {
        ...
        try {
          ...
          const {svg} = await mermaid.render(`mermaid-graph-${Math.random().toString(36).substring(7)}`, content);
  ...
  const getSvgString = (): string | null => {
    ...
    if (svgElement) {
        ...
        if (!clonedSvgElement.getAttribute('width') || !clonedSvgElement.getAttribute('height')) {
        ...
        if (firstChild instanceof SVGRectElement && firstChild.getAttribute('fill') && 
            (firstChild.getAttribute('fill')?.toLowerCase() === 'white' || firstChild.getAttribute('fill') === '#FFFFFF')) {
            hasWhiteBg = true;
        }
  ...
  const handleExport = async (format: "png" | "pdf") => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({ title: "Export Successful", description: `Diagram exported as ${format.toUpperCase()}.` });
      } else {
        const errorData = await response.json();
        toast({ title: "Export Failed", description: errorData.error || response.statusText, variant: "destructive" });
  ...
  return (
    <Card className="overflow-hidden shadow-lg">
      <CardHeader>
        ...
        {error && <CardDescription className="text-destructive pt-1">{error}</CardDescription>}
      </CardHeader>
      <CardContent>
        <ClientOnly fallback={FallbackDisplay}>
          {type === "mindmap" && (
            <svg ref={svgRef} className="w-full h-96 md:h-[500px] bg-card rounded-md shadow-inner data-ai-hint='mindmap abstract'"></svg>
          )}
          {type === "mermaid" && (
             <div ref={mermaidContainerRef} className="w-full min-h-80 md:min-h-[400px] p-2 bg-card rounded-md shadow-inner flex items-center justify-center overflow-auto data-ai-hint='gantt chart'">
              {/* Mermaid will render here. Fallback text if content is empty or invalid. */}
              {!content.includes("gantt") && !content.includes("graph") && !error && (
      ...
      <CardFooter className="flex flex-wrap gap-2 border-t pt-4">
        <Button
            ...
            className="shadow-sm"
        >
          {isExportingPng ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <ImageIcon className="mr-2 h-4 w-4" />}
          Export PNG
        </Button>
        <Button
            variant="outline"
            ...
            onClick={() => {
                toast({ 
                    ...
                    description: "Server-side PDF export is complex. For now, try printing to PDF from your browser (Ctrl/Cmd + P), or implement a client-side PDF solution like jsPDF for basic SVGs.",
                    duration: 8000,
                });
            }}
